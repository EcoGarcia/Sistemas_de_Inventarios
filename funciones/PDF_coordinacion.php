...
if (isset($_GET['identificador_coordinacion'])) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $queryUsuario = "SELECT Fullname FROM usuarios_coordinacion WHERE Fullname_direccion = ?";
        $stmtUsuario = mysqli_prepare($conn, $queryUsuario);
...
$html1= '<table border="0">
    <tr>
        ...
        <tr>
            ...
            <td style="padding: 30px; height: auto;">' . $row['caracteristicas'] . '</td>
        </tr>
        <tr>
    ...
    <tr>
        ...
        <td style="padding: 30px; height: 20px;">' . $row['serie'] . '</td>
    </tr>
    <tr>
...
<tr>
    ...
    // Salida del PDF
    $pdf->Output('respaldos_coordinacion' . $identificador_coordinacion . '.pdf', 'I');
    mysqli_close($conn);
} else {

...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST["userId"]) && isset($_POST["newUsername"]) && isset($_POST["identificadorCoordinacion"])) {
        ...
        $updateQuery = "UPDATE respaldos_coordinacion SET comentarios = '$newUsername' WHERE id = $userId AND identificador_coordinacion = $identificadorCoordinacion";

        if (mysqli_query($conexion, $updateQuery)) {
            $notification_message = "Cambio exitoso";
            echo "<script>
                alert('$notification_message');
                window.location.href = '../inventario/inventarios_coordinacion_admin.php?identificador_coordinacion=$identificadorCoordinacion';
            </script>";
        } else {
